...
const embeddingFunction = new OpenAIEmbeddingFunction({
    openai_api_key: my_api_key, // Replace with your actual API key
    openai_model: "text-embedding-3-small"    
});

const collectionName = "test-infos";
...
async function askQuestion() {
    const question = 'tell capital of pakistan? What does Alexandra Thompson like to do in her free time?';
    const collection = await getCollection();
    ...
    if (relevantInfo) {
        const openai = new OpenAI({
            apiKey: my_api_key // Ensure your API key is set here
        });
        const response = await openai.chat.completions.create({
            model: 'gpt-3.5-turbo',
            temperature: 0,
            messages: [
                { role: 'assistant', content: `Answer the next question using this information: ${relevantInfo}` },
